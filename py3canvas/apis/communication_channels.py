...
class CommunicationChannelsAPI(BaseCanvasAPI):
    ...
    def create_communication_channel(self, communication_channel_address, communication_channel_type, user_id, communication_channel_token=None, skip_confirmation=None):
        """
    ...
    def delete_push_notification_endpoint(self):
        ...
        self.logger.debug("DELETE /api/v1/users/self/communication_channels/push with query params: {params} and form data: {data}".format(params=params, data=data, **path))
        return self.generic_request("DELETE", "/api/v1/users/self/communication_channels/push".format(**path), data=data, params=params, single_item=True)
